...
| `abae837a50d3a50577e4a458e2bbf57e64312af3` | Partially aligned | Canonical native `replaceCallback()` and invocation-snapshot semantics are present; successor generation is built before disturbing the old one; per-entry detach, API102 legacy-API isolation and multi-user ownership are present. The canonical process-wide asynchronous reload channel/timeout model and permanent old-generation context freeze are not yet ported. |
...
The integration branch also contains fixes that are not claimed as upstream code:
...
- `BaseInvoker` now uses the canonical API102 native callback snapshot as the invocation boundary; a callback already snapshotted cannot disappear because another thread replaced its handle.
